...
contract TwapOracle {
  ...
  function activateTwap(address _pair, address _keeper) external {
    ...
    (
      uint112 reserve0,
      uint112 reserve1,
      uint32 pairTimestampLast
    ) = _pairContract.getReserves();
    uint32 blockTimestamp = uint32(block.timestamp % 2 ** 32);
    uint256 p0 = _pairContract.price0CumulativeLast();
    uint256 p1 = _pairContract.price1CumulativeLast();
    unchecked {
      if (
        pairTimestampLast != blockTimestamp && reserve0 != 0 && reserve1 != 0
      ) {
  ...
  function _decodePriceX112(
    ...
    if (amountIn != 0 && product / amountIn != uint256(priceX112))
