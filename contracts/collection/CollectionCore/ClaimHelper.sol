...
abstract contract ClaimHelper is Ownable, ERC1155, CollectionStorage {
    ...
    event ClaimRoundCreated(uint256 roundId, uint128 startTime, uint128 endTime, uint256 usdtPerNft);
    event Claimed(uint256 roundId, address account, uint256 tokenId, uint256 amount);
    ...
    function _enableMintAndClaim(uint128 startTime, uint256 usdtAmount) internal {
        ...
        uint16 maxMintsPerToken = (roundId == 1) ? 1 : claimRounds[roundId - 1].maxMintsPerToken * 2;
     ...
     */
    function _handleClaimPayment(address payer, uint256 price, uint256 quantity) internal {
     ...
     */
    function _handleMintForClaim(address to, uint256 id, uint256 amount, uint256 roundId) internal {
     ...
     */
    function _validateClaimTokens(uint256 roundId, address to, uint256 id, uint256 amount) internal view {
        ...
        require(currentTime >= round.startTime && currentTime <= round.endTime, "not in claim period");
        ...
        require(alreadyClaimed + amount <= preRoundBalance, "already claimed max for holder");

        uint256 totalMinted = mintedInRound[roundId][id];
        require(totalMinted + amount <= round.maxMintsPerToken, "exceeds claim limit");
    }
     ...
     */
    function _getRemainingToClaim(uint256 roundId, uint256 tokenId) internal view returns (uint256) {
