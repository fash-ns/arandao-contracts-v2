    ...
    
{
    ...
    function createOrder(
        ...
        _createOrder(buyerAddress, parentAddress, position, amounts, totalAmount, true);
    }
    ...
    function migrateOrder(
        ...
        _createOrder(buyerAddress, parentAddress, position, amounts, totalAmount, false);
    }

    function setMintArcArgs(uint256 _lastWeekDnmMintAmount, uint256 _dnmMintWeekNumber) external onlyMigrateOperator {
        lastWeekDnmMintAmount = _lastWeekDnmMintAmount;
        dnmMintWeekNumber = _dnmMintWeekNumber;
    }
        ...
        if (!user.migrated && user.fvEntranceMonth != 0) {
            ...
            if (user.fvEntranceMonth + 12 > month) {
                for (uint8 i = 1; i < 12; i++) {
                    uint256 requiredBvForFastValue = ((minBv * (12 ** i)) /
                        (10 ** i));
    ...
    function withdrawFastValueShare(uint256 selectedMonth) public nonReentrant {
        ...
        bool isPaymentSuccessful = _transferPaymentToken(
            msg.sender,
            userFvShare
        );
        ...
        emit CoreLib.MonthlyFastValueWithdrawn(
            userId,
            selectedMonth,
            userFvShare
        );
    ...
    function _calculateCommissionForPeriod(
        ...
        for (uint8 pairIndex = 0; pairIndex < 3; pairIndex++) {
            ...
            if (currentSteps == maxSteps) {
                ...
                if (isWeekly) {
                    emit CoreLib.UserWeeklyFlushedOut(
                        userId,
                        periodDayNumber / 7
                    );
                } else {
    ...
    function withdrawCommission(uint256 amount) external nonReentrant {
        ...
        require(
            isTxSuccessful,
            "Error while transferring payment token to user"
        );
    ...
    function calculateUserWeeklyArc() public nonReentrant {
        ...
        require(
            _getWeeklyBv(passedWeekNumber) > 0,
            "No BV recorded for the week"
        );
        ...
        emit CoreLib.UserArcShareCalculated(
            userId,
            passedWeekNumber,
            userDnmShare
        );
