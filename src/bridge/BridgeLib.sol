...
library BridgeLib {
    ...
    event UvmBridgedByUser(address userAddress, uint256 amount, uint256 totalBridgedArc);
    event DnmBridgedByUser(address userAddress, uint256 amount, uint256 totalBridgedArc);
    event ArusenseTokenBridgedByUser(address userAddress, uint256 tokenId, uint256 totalBridgedArc);
    event WrapperTokenBridgedByUser(address userAddress, uint256 tokenId, uint256 totalBridgedArc);
    ...
    event StakeYieldBridgedByUser(address userAddress, uint256 stakeId, uint256 uvmAmount, uint256 totalBridgedArc);
    ...
    function validateDeadline(uint256 constructionTime) internal view {
        require(block.timestamp < constructionTime + (90 days), "The time to bridge has been past.");
    }
    ...
    function calculateNewDnmFromOldDnm(uint256 dnmAmount) internal pure returns (uint256) {
        return dnmAmount / 10;
    }
    ...
    function transferERC20From(
        ...
        if (from == address(this)) {
            ...
            success = IERC20(tokenContract).transferFrom(from, to, amount);
        }
    ...
    function getUvmAmountByWrapperTokenType(uint8 _type) internal pure returns (uint256) {
        if (_type == 1) {
            return 680 ether;
        } else if (_type == 2) {
            return 340 ether;
