...
interface IStakeMeta {
    ...
    function stakeFrom(address staker, uint256 dnmAmount, uint256 wrapper_id, uint256 stake_duration) external;
    function stake(uint256 dnmAmount, uint256 land_id, uint256 stake_duration) external;
    ...
    function getUserStakeIds(address user_address) external view returns (uint256[] memory);
    function getUserStake(address user_address) external view returns (StakePlan[] memory);
    ...
    function getUser(address user_address) external view returns (uint256, bool, uint256);
    ...
    function calculateRewardForStake(uint256 stake_id) external view returns (uint256);
    function withdrawAllowance(StakePlan memory sp) external view returns (uint256);
    function calculateReward(uint16 plan_type, uint256 dnm_amount, uint256 start, uint256 end, uint256 stake_duration)
        external
        view
        returns (uint256);
