...
abstract contract YieldPoolCore is YieldPoolStorage {
    ...
    function _batchStakeFor(address[] calldata users, uint256[] calldata amounts) internal {
        if (block.timestamp > deployTime + BATCH_STAKE_WINDOW) revert YieldPoolErrors.StakeForWindowClosed();
        if (lpModeActive) revert YieldPoolErrors.ArcStakingDisabled();
        ...
        for (uint256 i; i < len;) {
            ...
            uint256 stakeId = nextStakeId++;
            stakes[stakeId] = Stake({amount: amount, rewardDebt: amount * acc / PRECISION, owner: user, active: true});
            _userStakeIds[user].push(stakeId);
            totalAmount += amount;
