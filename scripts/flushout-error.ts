...
const getTxData = async () => {
    const tx = await ethers.provider.getTransaction('0x5b2032d7d4a2f995d4fc598932ecdfb1550df08e8e81409d43051ac2db60f3da');
...
const getTxNonce = async () => {
    const tx = await ethers.provider.getTransaction('0x26c2d96976976bc602b3f9328abb89bc8ba02f40a712f4c6f00f9ca0816ac8a2');
...
const main = async () => {
    const fileData = JSON.parse(
      fs.readFileSync(
          path.resolve("./personal", "addonBvs.json")
      ).toString()
    );
    ...
    for (let i = 1; i < fileDataLen; i++) {
      ...
      const tx = await coreContract.updateUserById(BigInt(item.userId), inputData, {
