...
const daiContractData = getContractData("usdt");
...
const mintArcFromCore = async () => {
  await coreContract.mintArc(
    "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC",
    parseEther("10"),
  );
  const balance = await arcContract.balanceOf(
    "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC",
  );
  console.log(balance);
};

const mintArcFromCoreAfterDevMode = async () => {
  await coreContract.revokeDevMode();
  await coreContract.mintArc(
    "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC",
    parseEther("10"),
  );
...
const mintWeeklyArc = async () => {
  ...
  const getGlobalDailySteps1 = await (coreContract as any).getGlobalDailySteps(
    126,
  );
  const getGlobalDailySteps2 = await (coreContract as any).getGlobalDailySteps(
    127,
  );
  const getGlobalDailySteps3 = await (coreContract as any).getGlobalDailySteps(
    128,
  );
  const getGlobalDailySteps4 = await (coreContract as any).getGlobalDailySteps(
    129,
  );
  const getGlobalDailySteps5 = await (coreContract as any).getGlobalDailySteps(
    130,
  );
  const getGlobalDailySteps6 = await (coreContract as any).getGlobalDailySteps(
    131,
  );
  const getGlobalDailySteps7 = await (coreContract as any).getGlobalDailySteps(
    132,
  );
...
const getSellerArcShare = async (sellerId: number, weekNo: number) => {
  ...
  try {
    ...
    // await tx.wait();
    const sellerBalance = await arcContract.balanceOf(
      "0x6Bd6a164Bc92632946C33346F0E20B083bcbEfD5",
    );

    const expectedAmount =
      (((mintedArc * BigInt(5)) / BigInt(100)) * sellerWeeklyBv) /
      totalWeeklyBv;
...
const getBuyerArcShare = async (weekNo: number) => {
  ...
  try {
    ...
    const expectedAmount =
      (((mintedArc * BigInt(35)) / BigInt(100)) * userWeeklyBv) / totalWeeklyBv;
...
const getNetworkerArcShare = async (weekNo: number) => {
  ...
  try {
    ...
    const expectedAmount =
      (((((mintedArc * BigInt(60)) / BigInt(100)) * BigInt(2)) / BigInt(12)) *
        BigInt(70)) /
      BigInt(100);
...
const transferArc = async () => {
  await arcContract.transfer(
    "0x7d9e97ef4089b73eadfc28e83f7c8b90fc63b22c",
    parseEther("5"),
  );
};
...
const mintWeeklyArcWithLog = async () => {
  const arcBalanceBefore = await arcContract.balanceOf(
    "0x282B01760c0300e73A88d5466D6DdDAC16Fb7C77",
  );
  await coreContract.mintWeeklyARC();
  const arcBalanceAfter = await arcContract.balanceOf(
    "0x282B01760c0300e73A88d5466D6DdDAC16Fb7C77",
  );
...
const main = async () => {
  await mintArcFromCoreAfterDevMode();
  // await mintWeeklyArc();
