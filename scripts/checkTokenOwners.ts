...
const collectionContract = new ethers.BaseContract(
  "0xc1A26F85582308753487C7F0Ec672A2aA37eeabE",
  collectionAbi.abi,
  signers[0],
...
for (let i = 0; i < 5; i++) {
  ...
  const balance = await collectionContract.balanceOfBatch(
    dataSet.map((ind) => ind.walletAddress),
    dataSet.map((ind) => ind.blockchainTokenId),
  );
